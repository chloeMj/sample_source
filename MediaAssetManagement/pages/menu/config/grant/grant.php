...
(function(){
	...
	Ariel.config.Category_Grant = Ext.extend(Ext.Panel, {
		...
		initComponent: function(config){
			...
			//
			//									if( parseInt(nameArray[1]) & parseInt( select.get('group_grant') ) )//권한 체크
			//									{
			...
			this.items = [{
				...
				buttons: [{
					...
					handler: function() {
						...
						var ud_content_form		= that.get(0).get(1).get(0).get(0);
						var member_group_form	= that.get(0).get(1).get(1).get(0);
						var grant_content		= that.get(0).get(1).get(2).get(0);
						var group_grant_form	= that.get(0).get(1).get(3).get(0);
						var grid = that.get(0).get(0).get(0);
				...
				items: [{
					...
					items: [{
						...
						cls: 'grid_title_customize proxima_grid_header',
						border: false,
						enableHdMenu: false,
						loadMask: true,
						autoShow: true,
						...
						columns: [
							...
							{ header: "사용자정의콘텐츠", sortable: true, dataIndex: 'ud_content_title' },
							{ header: "사용자 그룹",  sortable: true, dataIndex: 'member_group_name' },
							{ header: "콘텐츠 권한", sortable: true, dataIndex: 'grant_text' }*/
							{ header: _text('MN02046'), sortable: true, dataIndex: 'ud_content_title' },
							{ header: _text('MN02047'),  sortable: true, dataIndex: 'member_group_name' },
							{ header: _text('MN01003'), sortable: true, dataIndex: 'grant_access_text' },
							{ header: _text('MN02194'), sortable: true, dataIndex: 'grant_text' }
						],
						sm: new Ext.grid.RowSelectionModel({
						}),
						...
						listeners: {
							rowclick: function(self, idx, e){
								...
								var ud_content_form = that.get(0).get(1).get(0).get(0);
								var member_group_form = that.get(0).get(1).get(1).get(0);
								var grant_access_form = that.get(0).get(1).get(2).get(0);
								var group_grant_form  = that.get(0).get(1).get(3).get(0);

								ud_content_form.reset();
								member_group_form.reset();
								grant_access_form.reset();
								group_grant_form.reset();
								...
								grant_access_form.items.each(function(i) {
									...
									// 권한 체크
									if (parseInt(nameArray[1]) & parseInt(select.get('grant_access'))) {
										i.suspendEvents(false);
										grant_access_form.setValue(i.getName(), true);
										i.resumeEvents();
									}
								});
						...
						items: [{
							...
							<?php
								$grant_query = "
										...
										WHERE	B.CODE = 'grant_content'
										ORDER BY A.ID
								";
								$grant_list = $db->queryAll($grant_query);
								
								$items_access_grant = array();
								foreach ($grant_list as $grant) {
									array_push($items_access_grant, "{boxLabel: '{$grant['name']}', name: 'access-{$grant['code']}'}");
								}
								echo join($items_access_grant, ',');
							?>
							]
						}]
					}]
